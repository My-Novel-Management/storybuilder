...
class Writer(BaseActor):
    ...
    def be(self, *args, **kwargs) -> Action:
        return Action(*args,
                subject=self.src, act_type=ActType.BE,
                note=self._countIf(args), **kwargs)

    def come(self, *args, **kwargs) -> Action:
        return Action(*args,
                subject=self.src, act_type=ActType.COME,
                note="1", **kwargs)

    def destroy(self, *args, **kwargs) -> Action:
        return Action(*args,
                subject=self.src, act_type=ActType.DESTROY,
                note=self._countIf(args), **kwargs)

    def discard(self, *args, **kwargs) -> Action:
        return Action(*args,
                subject=self.src, act_type=ActType.DISCARD,
                note=self._countIf(args), **kwargs)

    def do(self, *args, **kwargs) -> Action:
        return Action(*args,
                subject=self.src, act_type=ActType.ACT, **kwargs)

    def explain(self, *args, **kwargs) -> Action:
        return Action(*args,
                subject=self.src, act_type=ActType.EXPLAIN, **kwargs)

    def go(self, *args, **kwargs) -> Action:
        return Action(*args,
                subject=self.src, act_type=ActType.GO,
                note="1", **kwargs)

    def have(self, *args, **kwargs) -> Action:
        return Action(*args,
                subject=self.src, act_type=ActType.HAVE,
                note=self._countIf(args), **kwargs)
    ...
    def move(self, *args, **kwargs) -> Action:
        return Action(*args, subject=self.src, act_type=ActType.MOVE, **kwargs)

    def takeoff(self, *args, **kwargs) -> Action:
        return Action(*args, subject=self.src, act_type=ActType.TAKEOFF,
                note=self._countIf(args), **kwargs)

    def talk(self, *args, **kwargs) -> Action:
        return Action(Shot(*args, isTerm=True), subject=self.src, act_type=ActType.TALK, **kwargs)

    def talking(self, *args, **kwargs) -> Action:
        return Action(Shot(*args), subject=self.src, act_type=ActType.TALK, **kwargs)
    ...
    def wear(self, *args, **kwargs) -> Action:
        return Action(*args, subject=self.src, act_type=ActType.WEAR,
                note=self._countIf(args), **kwargs)
