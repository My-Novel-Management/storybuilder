...
class CompilerTest(unittest.TestCase):
    ...
    def test_conv_from_tag(self):
        data = [
                # (src, head, nums, is_cmt, is_plot, expect, exp_nums)
                (True, SCode(None, SCmd.TAG_BR, (), ''), '#', (1,1,1),
                    True, False,
                    '\n\n', (1,1,1)),
                ]
        def checker(src, head, nums, is_cmt, is_plot, expect, exp_nums):
            tmp = cp.Compiler()._conv_from_tag(src, head, nums, is_cmt, is_plot)
